...
{ //Wenn sende-Button gedrückt wurde
    ...
    if ($erg3->num_rows)
    { //Wenn min. 1 Ergebnis zurückkommt
    ...
    foreach ($user as $u)
    {
        ...
        if ($u->benutzername == $benutzername or $u->email == $email)
        { // username oder email existiert schon
    ...
    if ($eintragen)
    { //wenn eintragen funktioniert hat, dann start session
